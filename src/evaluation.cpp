...
evaluation::evaluation(const std::vector<expression> &exprs)
    ...
    exprs_(exprs)
{
    //printf("expressions in evaluation\n");
}
...
int evaluation::execute()
{
    ...
    for (auto iter = exprs_.begin() ; iter != exprs_.end() ; ++iter){
        ...
        /*std::cout << iter->op_name_ <<"\n";
        auto it = variables_.find(iter->op_name_);
            if (it != iter->op_param_.end()) {
            // Key exists, so you can safely access it
            std::cout << it->second << "\n";
            } else {
        ...
        if(iter->op_type_ == "Input"){
            ...
            //values[iter->expr_id_] = sum1 + sum2;
            //result_ = sum1 + sum2;
        }
            ...
            /*for (const auto& expr : exprs_) {
                if (expr.expr_id_ == iter->inputs_[0]) {
                    mul1 = values[expr.expr_id_];
                }
                if (expr.expr_id_ == iter->inputs_[1]) {
                    mul2 = values[expr.expr_id_];
                }
            }*/
            ...
            //values[iter->expr_id_] = mul1 * mul2;
            //result_ = mul1 * mul2;
        }
            ...
            N=sub1.get_elements();            
            for (size_t i=0 ; i<N ; i++){
                suma.push_back(sub1.at(i) - sub2.at(i));
            }                       
            sub2.set_data_array(suma);
            values[iter->expr_id_]=sub2;

            //values[iter->expr_id_] = sub1 - sub2;
            //result_ = sub1 - sub2;
        }
        //result_ = values[iter->expr_id_];
